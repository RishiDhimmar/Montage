...
const Model3D: React.FC<Model3DProps> = observer(({ id, nodes }) => {
  ...
  useEffect(() => {
    const disposer = reaction(
      ...
      async (selectedTextures) => {
        ...
        try {
          ...
          if (selectedTextures["0"]?.materialUrl) {
            newTextures["External Wall"] = await loadTexture(selectedTextures["0"].materialUrl);
          }
          if (selectedTextures["2"]?.materialUrl) {
            newTextures["Interior Wall"] = await loadTexture(selectedTextures["2"].materialUrl);
          }
          if (selectedTextures["3"]?.materialUrl) {
            newTextures["Interior Floor"] = await loadTexture(selectedTextures["3"].materialUrl);
          }
    ...
    return () => disposer(); // Cleanup reaction on unmount
  }, []);

  const position = modelStore.getPosition(id) || [0, 0, 0];
  const rotation = modelStore.getRotation(id) || [0, 0, 0]; 
  ...
  return (
    <group position={position as [number, number, number]} rotation={rotation as [number, number, number]} scale={scale}>
      {Object.entries(nodes).map(([key, node]) => {
        ...
        const material = assignedTexture
          ? new THREE.MeshStandardMaterial({ map: assignedTexture, roughness: 1 })
          : node.name.includes("Node")
          ? materials.cyan
          : node.material ?? undefined;

        return (
          <mesh key={key} castShadow receiveShadow geometry={node.geometry} material={material} matrixAutoUpdate={false} matrix={node.matrixWorld} />
        );
      })}
    </group>
  );
