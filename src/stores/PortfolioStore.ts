...
interface Design {
  ...
  designImage: string;
  monogramImage: string;
  moduleArr: unknown[]; // If you know the exact type, replace `unknown` with it
}
...
class PortFolioStore {
  ...
  setPortfolios(apiData: Portfolio[]) {
    runInAction(() => {
      this.portfolios = apiData.map((portfolio) => ({
        id: portfolio.id,
        name: portfolio.name,
      ...
      this.selectedPortfolio.set(this.portfolios.length > 0 ? this.portfolios[0] : null);
    });
  }
  ...
  addPortfolio(newPortfolio: Portfolio) {
    runInAction(() => {
      this.portfolios.push(newPortfolio);
      this.selectedPortfolio.set(newPortfolio);
    });
  }
  ...
  async fetchDesigns() {
    ...
    try {
      const data: { portFolios: Portfolio[] } = await fetchData(`${BASE_URL}/portfolios`);
      console.log("API Response:", data);
